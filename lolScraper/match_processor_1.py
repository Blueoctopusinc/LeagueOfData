...
euwApiController = ApiHandler("euw1", 'RGAPI-58edc1f7-6da6-4e66-a9c2-16d806d07a82')

matches = pd.read_csv('/home/joshua/PycharmProjects/dataCollector/1', usecols=[0], names=['match'])
...
for i in matches['match']:
    if count < limit:
        ...
        if match != -1:
            print("Matches processed: ", count)
            dataset.extend(match)
            processed_matches.append(i)
            count +=1
            df_processed_matches = pd.DataFrame(processed_matches)
            df_dataset = pd.DataFrame(dataset)
            df_dataset.to_csv("dataset_1")
            df_processed_matches.to_csv("processed_matches_1")
        if match == -1:
